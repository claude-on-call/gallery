...
describe(SearchRepository.name, () => {
  ...
  describe('locationPresence', () => {
    ...
    it('narrows the timeline buckets, not just metadata search', async () => {
      ...
      const all = await assets.getTimeBuckets({ userIds: [user.id] }, auth);
      const noGps = await assets.getTimeBuckets({ userIds: [user.id], locationPresence: 'noGps' }, auth);
      const noPlaceName = await assets.getTimeBuckets({ userIds: [user.id], locationPresence: 'noPlaceName' }, auth);
    ...
    it('keeps getTimeBuckets and getTimeBucket in agreement under a locationPresence filter', async () => {
      ...
      const buckets = await assets.getTimeBuckets({ userIds: [user.id], locationPresence: 'noPlaceName' }, auth);
