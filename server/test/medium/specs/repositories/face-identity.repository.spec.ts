...
const seedTypedIdentity = async (
  ctx: ReturnType<typeof setup>['ctx'],
  sut: FaceIdentityRepository,
  input: { userId: string; type?: string; species?: string; name?: string; assets: number; embedded?: boolean },
) => {
  const { person } = await ctx.newPerson({
    ownerId: input.userId,
    name: input.name ?? '',
  ...
  for (let index = 0; index < input.assets; index++) {
    ...
    const { assetFace } = await ctx.newAssetFace({ assetId: asset.id, personGroupId: person.personGroupId });
    await sut.linkFace({ assetFaceId: assetFace.id, identityId: identity.id, source: 'owner-person' });
    // A pet_search row marks a pet as an individual the re-ID model identified; a species bucket
    // never has one.
    if (input.embedded) {
      await ctx.database
...
const seedBirthdayPermissionWorld = async (ctx: ReturnType<typeof setup>['ctx'], sut: FaceIdentityRepository) => {
  ...
  const { assetFace: aliceFace } = await ctx.newAssetFace({
    assetId: aliceAsset.id,
    personGroupId: alicePerson.personGroupId,
  });
  ...
  await ctx.database
    ...
    .where('personGroupId', '=', davePerson.personGroupId)
    .execute();
  const { asset: daveAsset } = await ctx.newAsset({ ownerId: dave.id, visibility: AssetVisibility.Timeline });
  const { assetFace: daveFace } = await ctx.newAssetFace({
    assetId: daveAsset.id,
    personGroupId: davePerson.personGroupId,
  });
...
describe(FaceIdentityRepository.name, () => {
    ...
    try {
      ...
      for (let index = 0; index < 20; index++) {
        ...
        const { assetFace } = await ctx.newAssetFace({
          assetId: asset.id,
          personGroupId: dominantPerson.personGroupId,
        });
  ...
  it('reports mixed and duplicate space-person links as repairable backfill work', async () => {
    ...
    try {
      ...
      const { assetFace: firstFace } = await ctx.newAssetFace({
        ...
        personGroupId: firstPerson.personGroupId,
      });
  ...
  it('backfills personal identities idempotently and pages by cursor', async () => {
    ...
    try {
      ...
      expect(people.find((person) => person.personGroupId === firstPerson.personGroupId)?.identityId).toBe(
  ...
  it('repairs only mismatched personal face groups when a stale person still has correctly linked faces', async () => {
    ...
    try {
      ...
      const { person: sourcePerson } = await ctx.newPerson({ ownerId: user.id });
      const { asset } = await ctx.newAsset({ ownerId: user.id });
      const { assetFace: sourceLinkedFace } = await ctx.newAssetFace({
        assetId: asset.id,
        personGroupId: sourcePerson.personGroupId,
      });
      const { assetFace: targetLinkedFace } = await ctx.newAssetFace({
        assetId: asset.id,
        personGroupId: sourcePerson.personGroupId,
      });
      const targetIdentity = await sut.ensurePersonIdentity(targetPerson.personGroupId);
  ...
  it('resolves a space-set birthday via the single-person view', async () => {
    ...
    try {
      ...
      expect(result).toEqual(
        expect.objectContaining({ id: person.personGroupId, name: 'Ina', birthDate: '2014-02-14' }),
      );
    } finally {
      await ctx.database.deleteFrom('user').where('id', '=', user.id).execute();
    }
  ...
  it('returns the person with a null birthday when no profile has one', async () => {
    ...
    try {
      const result = await sut.getAccessiblePeople(user.id, { withHidden: false, page: 1, size: 50 });
      expect(result.people).toEqual([
        expect.objectContaining({ id: person.personGroupId, name: 'Ina', birthDate: null }),
      ]);
    } finally {
      await ctx.database.deleteFrom('user').where('id', '=', user.id).execute();
    }
  ...
  it('does not surface a birthday from a hidden space profile unless withHidden is set', async () => {
    ...
    try {
      ...
      expect(hiddenIncluded.people).toEqual([
        expect.objectContaining({ id: person.personGroupId, birthDate: '2014-02-14' }),
      ]);
    } finally {
      await ctx.database.deleteFrom('user').where('id', '=', user.id).execute();
    }
  ...
  it('filters unnamed identity-grouped people below the configured minimum face count', async () => {
    ...
    try {
      ...
      for (let index = 0; index < 3; index++) {
        ...
        const { assetFace } = await ctx.newAssetFace({
          assetId: asset.id,
          personGroupId: eligiblePerson.personGroupId,
        });
  ...
  describe('getAccessiblePersonStatistics', () => {
    it('counts owned and timeline shared-space identity faces once each', async () => {
      ...
      try {
        ...
        const { assetFace: ownedFace } = await ctx.newAssetFace({
  ...
  describe('personal identity repair embedding guard', () => {
    it('does not move a face onto a person it does not resemble during personal backfill', async () => {
      ...
      try {
        ...
        const { asset } = await ctx.newAsset({ ownerId: user.id });
        const { assetFace: corruptFace } = await ctx.newAssetFace({
          assetId: asset.id,
    ...
    it('still moves a face that resembles the target person during personal backfill', async () => {
      ...
      try {
        ...
        const { asset } = await ctx.newAsset({ ownerId: user.id });
        const { assetFace: resemblingFace } = await ctx.newAssetFace({
          assetId: asset.id,
          personGroupId: personB.personGroupId,
        });
        await ctx.database
    ...
    it('does not block a face once the target cluster is already contaminated (documents a known limitation)', async () => {
      ...
      try {
        ...
        await sut.ensurePersonIdentity(personB.personGroupId);
        const { asset } = await ctx.newAsset({ ownerId: user.id });
        const { assetFace: bFace } = await ctx.newAssetFace({
          assetId: asset.id,
          personGroupId: personB.personGroupId,
        });
        await ctx.database
  ...
  describe('demoteManualFaceLinks (F20)', () => {
    it('demotes only the requested manual links, chunked, without a bind-parameter error on a huge request', async () => {
      ...
      const { asset: assetA } = await ctx.newAsset({ ownerId: user.id });
      const { assetFace: manualFaceA } = await ctx.newAssetFace({
        assetId: assetA.id,
        personGroupId: person.personGroupId,
      });
      ...
      const { asset: assetB } = await ctx.newAsset({ ownerId: user.id });
      const { assetFace: manualFaceB } = await ctx.newAssetFace({
        assetId: assetB.id,
        personGroupId: person.personGroupId,
      });
      ...
      const { assetFace: untouchedManualFace } = await ctx.newAssetFace({
        assetId: assetC.id,
        personGroupId: person.personGroupId,
      });
  ...
  describe('getPersonVerdictTokens (H6)', () => {
    it('resolves tokens for a person among a personId list far larger than the bind-parameter ceiling', async () => {
      ...
      const filler = Array.from({ length: 70_000 }, () => randomUUID());
      const tokens = await sut.getPersonVerdictTokens([
        person.personGroupId,
        personWithoutIdentity.personGroupId,
        ...filler,
      ]);

      expect(tokens.get(person.personGroupId)).toEqual([`identity:${identity.id}`, `person:${person.personGroupId}`]);
      expect(tokens.get(personWithoutIdentity.personGroupId)).toEqual([
        `person:${personWithoutIdentity.personGroupId}`,
      ]);
    });
  });
});
...
describe('FaceIdentityRepository getAccessiblePeople type filter', () => {
  it('returns only pets when type is pet', async () => {
    ...
    expect(ids).toContain(pet.personGroupId);
    expect(ids).not.toContain(human.personGroupId);
  });
  ...
  it('returns only humans when type is person', async () => {
    ...
    const ids = result.people.map(({ id }) => id);
    expect(ids).toContain(human.personGroupId);
    expect(ids).not.toContain(pet.personGroupId);
  });

  it('surfaces a single-face individual under type=pet, despite the minimumFaceCount gate', async () => {
    const { sut, ctx } = setup();
    const { user } = await ctx.newUser();
    const { person: pet } = await seedTypedIdentity(ctx, sut, {
      userId: user.id,
    ...
    expect(result.people.map(({ id }) => id)).toContain(pet.personGroupId);
  });
  ...
  it('counts a single-face individual in the header total under type=pet', async () => {
    const { sut, ctx } = setup();
    const { user } = await ctx.newUser();
    await seedTypedIdentity(ctx, sut, { userId: user.id, type: 'pet', species: 'dog', assets: 1, embedded: true });
  ...
  // The overview statistics feed the UNFILTERED header, which keeps the minimumFaces gate — so a
  // single-photo pet stays out of it, exactly as before this feature. Pinned so the "All view is
  // unchanged" promise cannot regress.
  it('keeps the minimumFaces gate in the overview statistics', async () => {
    const { sut, ctx } = setup();
    const { user } = await ctx.newUser();
    await seedTypedIdentity(ctx, sut, { userId: user.id, type: 'pet', species: 'dog', assets: 1, embedded: true });
  ...
  // Same for the face statistics: unfiltered, so the gate stands and the single-photo pet's face is
  // not counted as an assigned visible face.
  it('keeps the minimumFaces gate in the face statistics', async () => {
    const { sut, ctx } = setup();
    const { user } = await ctx.newUser();
    await seedTypedIdentity(ctx, sut, { userId: user.id, type: 'pet', species: 'dog', assets: 1, embedded: true });
    ...
    expect(stats.assignedVisibleFaceCount).toBe(0);
  });
  ...
  it('reports a total scoped to the type filter', async () => {
    ...
    await seedTypedIdentity(ctx, sut, {
      userId: user.id,
      type: 'pet',
      species: 'dog',
      name: 'Rex',
  ...
  it('excludes species buckets from type=pet', async () => {
    const { sut, ctx } = setup();
    const { user } = await ctx.newUser();
    const { person: bucket } = await seedTypedIdentity(ctx, sut, {
      userId: user.id,
      type: 'pet',
      species: 'bird',
      name: 'bird',
    ...
    const { person: individual } = await seedTypedIdentity(ctx, sut, {
      userId: user.id,
      type: 'pet',
      species: 'dog',
      assets: 2,
    ...
    const ids = result.people.map(({ id }) => id);
    expect(ids).toContain(individual.personGroupId);
    expect(ids).not.toContain(bucket.personGroupId);
  });

  it('keeps the minimumFaceCount gate on the unfiltered view', async () => {
    const { sut, ctx } = setup();
    const { user } = await ctx.newUser();
    const { person: individual } = await seedTypedIdentity(ctx, sut, {
      userId: user.id,
    ...
    const unfiltered = await sut.getAccessiblePeople(user.id, {
      withHidden: false,
      page: 1,
      size: 50,
      minimumFaceCount: 3,
    ...
    expect(unfiltered.people.map(({ id }) => id)).not.toContain(individual.personGroupId);
  });
  ...
  it('still hides an unnamed human with a single face', async () => {
    ...
    expect(result.people.map(({ id }) => id)).not.toContain(human.personGroupId);
  });
});
