...
describe('SharedSpaceService space face suggestions', () => {
  ...
  it('confirm creates a missing space identity, links the candidate face, and propagates to the owner without changing asset ownership (edges 26 and 31)', async () => {
    ...
    const face = await ctx.database
      ...
      .executeTakeFirstOrThrow();
    // Since §6.3.1 was revised (2026-08-25) a confirm propagates into the owner's layer, so
    // personGroupId is now SET -- to a person in the asset owner's own library carrying the space
    // person's name. Edge 31 is unchanged and is the point of the ownerId assertion: propagating a
    // face never transfers the ASSET.
