...
const verdictRow = (assetFaceId: string) =>
  defaultDatabase
    ...
    .select(['id', 'personGroupId', 'spacePersonId', 'identityId', 'status', 'source', 'actorId', 'distance'])
...
describe('face verdict merge durability (D1)', () => {
  ...
  it('a source negative outranks a survivor pending row', async () => {
    ...
    await facePersonVerdictRepository.upsertPending([
      { personGroupId: robert.personGroupId, assetFaceId: controlFaceId, distance: 0.4 },
    ]);
  ...
  it("S6.2: the promoted row carries the source verdict's source/actor and clears distance", async () => {
    ...
    await facePersonVerdictRepository.upsertPending([
      { personGroupId: robert.personGroupId, assetFaceId: faceId, distance: 0.4 },
    ]);
  ...
  it('S6.3 (pin): a source pending row never overwrites a survivor rejected row', async () => {
    ...
    const robertIdentity = await faceIdentityRepository.ensurePersonIdentity(robert.personGroupId);
    await facePersonVerdictRepository.upsertPending([
      { personGroupId: bob.personGroupId, assetFaceId: faceId, distance: 0.4 },
    ]);
    await facePersonVerdictRepository.markRejected(robert.personGroupId, faceId, {
      identityId: robertIdentity.id,
      source: 'cleanup',
  ...
  it('S6.7: promotion keeps the survivor identity when the source verdict carries none', async () => {
    ...
    await defaultDatabase
      ...
      .where('personGroupId', '=', robert.personGroupId)
  ...
  it('S6.9: a three-way merge (two negative sources into one pending survivor) ends in exactly one negative row', async () => {
    ...
    const faceId = await seedFace(ctx, user.id);
    await facePersonVerdictRepository.markRejected(bob.personGroupId, faceId, { source: 'cleanup', actorId: user.id });
    await facePersonVerdictRepository.markIgnored(carol.personGroupId, faceId, {
      source: 'suggestion',
      actorId: user.id,
    });
    await facePersonVerdictRepository.upsertPending([
    ...
    expect(rows[0].personGroupId).toBe(robert.personGroupId);
