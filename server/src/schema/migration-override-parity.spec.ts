...
// This migration creates shared_space_album_hidden_delete_audit (function + statement AFTER
// DELETE trigger), same shape as album_space_asset_delete_audit above: an unconditional
// `INSERT ... SELECT ... FROM "old"; RETURN NULL` fan-in to an audit table.
describe('1793000000000-AddSharedSpaceAlbumHidden override parity', () => {
  beforeEach(() => {
    capturedSql.length = 0;
  });

  it('executes and overrides the delete-audit function with DDL byte-identical to functions.ts', async () => {
    await upSharedSpaceAlbumHidden({} as any);

    const executedFunctionDdl = findSql('the delete-audit CREATE FUNCTION', (s) =>
      s.startsWith('CREATE OR REPLACE FUNCTION shared_space_album_hidden_delete_audit()'),
    );
    expect(executedFunctionDdl).toBe(shared_space_album_hidden_delete_audit.expression);

    const functionOverrideInsert = findSql('the delete-audit function override row', (s) =>
      s.includes(`VALUES ('function_shared_space_album_hidden_delete_audit'`),
    );
    expect(parseOverrideValue(functionOverrideInsert).sql).toBe(shared_space_album_hidden_delete_audit.expression);
  });

  // The trigger half. functions.ts has no counterpart to compare against (a trigger is declared by
  // the @AfterDeleteTrigger decorator on SharedSpaceAlbumHiddenTable, and importing src/schema under
  // this file's `kysely` mock blows up), so this pins the migration's two strings to each other and
  // trigger-override-parity.spec.ts pins the decorator's generated DDL to the same text. Together
  // they close the loop: decorator === executed DDL === override row.
  it('stores a trigger override row byte-identical to the trigger it executes', async () => {
    await upSharedSpaceAlbumHidden({} as any);
    ...
    const executedTriggerDdl = findSql('the delete-audit CREATE TRIGGER', (s) =>
      s.startsWith('CREATE OR REPLACE TRIGGER "shared_space_album_hidden_delete_audit"'),
    );
    const triggerOverrideInsert = findSql('the delete-audit trigger override row', (s) =>
      s.includes(`VALUES ('trigger_shared_space_album_hidden_delete_audit'`),
    );
