...
export const utils = {
  ...
  createPetWithEmbedding: async (
    ...
    assetId: string,
    name?: string,
  ): Promise<{ personGroupId: string; faceId: string }> => {
    if (!client) {
      throw new Error('Database client not connected');
    }

    const personGroupId = await utils.createPet(ownerId, species, name);
    const faceId = await utils.createFace({ assetId, personGroupId });
